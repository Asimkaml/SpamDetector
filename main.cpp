...
int main()
{
    ...
    while (!WindowShouldClose())
    {
        ...
        else
        {
            ...
            if (((key >= 32 && key <= 126) || (key >= 128 && key <= 255)) && strlen(message) < 255)
            {
        ...
        DrawTextBoxedSelectable(myFont, message, inputBox, 22, 1, true, DARKGRAY, 0, 0, WHITE, LIGHTGRAY, &scrollOffsetY);
        
        //showing range of characters
        string range = to_string(strlen(message)) + "/1024";
        DrawTextEx(myFont,range.c_str(), {650, 355}, 18, 0, BLACK);
